...
namespace Pingqu\OpenApiSdk\Core\Http;


use Pingqu\OpenApiSdk\Core\Exceptions\ServerException;
...
class HttpBase
{
    ...
    public static function curl(Request $request)
    {
        ...
        if (curl_errno($ch)) {
            throw new ServerException('pingqu-cloud-server is unavailable');
        }
        curl_close($ch);
